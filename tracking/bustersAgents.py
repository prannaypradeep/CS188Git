...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        referenceMark = 100000000000000
        ghostPos = None

        for dist in livingGhostPositionDistributions:
            posHolder = dist.argMax()
            distHolder = self.distancer.getDistance(pacmanPosition, posHolder)
            if referenceMark > distHolder:
                referenceMark = distHolder
                ghostPos = posHolder




        referenceMark2= 100000000000000
        theMove = None

        for a in legal:
            nextPos = Actions.getSuccessor(pacmanPosition, a)
            distHolder2 = self.distancer.getDistance(nextPos, ghostPos)
            if referenceMark2 > distHolder2:
                referenceMark2 = distHolder2
                theMove = a
        return theMove
